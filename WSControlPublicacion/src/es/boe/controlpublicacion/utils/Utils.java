package es.boe.controlpublicacion.utils;
...
import es.boe.controlpublicacion.wssecurity.WSSecurityException;
...
public final class Utils {
	...
	public static void firmaMensaje(SOAPMessageContext context, ContextoLlamada ctx) throws XMLDOMDocumentException, WSSecurityException, SOAPException
	{
		String mensajeSalida = XMLDOMUtils.getPrettyXMLText(context.getMessage().getSOAPPart());
		mensajeSalida=WSSecurityFactory.newConstructorResultado(ctx.getLogger(), ctx.getPreferencias(),new SoapClientHandler(ctx)).firmaMensaje(mensajeSalida);
		context.getMessage().getSOAPPart().setContent(new StreamSource(new StringReader(mensajeSalida)));
	}
	public static void generaRespuestaError(SOAPMessageContext context, String codigo, String mensaje) {
	       try {
	    	  SOAPMessage msg = context.getMessage();
	          SOAPBody soapBody = msg.getSOAPPart().getEnvelope().getBody();
	          soapBody.removeContents();
	          msg.getSOAPPart().getEnvelope().addNamespaceDeclaration("ns1", "http://www.boe.es/ServicioControlPublicacion");
	          QName respuestaName=soapBody.createQName("Respuesta", "ns1");
	          SOAPFactory f = SOAPFactory.newInstance();
	          SOAPElement respuesta = f.createElement(respuestaName);
	          Name idName  = f.createName("id");
	          SOAPElement id = respuesta.addChildElement(idName);
	          Name descripcionName = f.createName("descripcion");
	          SOAPElement descripcion = respuesta.addChildElement(descripcionName);
	          id.addTextNode(codigo);
	          descripcion.addTextNode(mensaje);
	          soapBody.addChildElement(respuesta);
